 ...
 * 列表里的颜色（见 TECH 4.4）。
 *
 * 大红（`#D6493C`，~5°）是 2026-09-26 换进来的：它顶替了青柠（127°）的位置，把色环
 * 的最暖端补齐——十色里从此有一个一眼可读的「红」，与珊瑚（偏橙的浅红）、玫红
 * （偏粉）都不混淆。红味取全，饱和与明度压在和珊瑚、玫红同一档灰调上，
 * 全饱和的 `#FF2A1E` 放进色板里会显得荧光（实渲染对比过）。青柠下架后，苔绿 86° → 松绿 168° 之间重新留出空档，两枚绿
 * 中间不再有第三枚绿，反而更好认。
 *
